...
function _normaliser(brut, referentielBanques) {
  ...
  return { champs: champs, confiances: conf };
}

/* Point d'entree. cacheGet/cacheSet : fonctions best-effort (cle->objet) fournies par
 * l'endpoint (Cosmos). Renvoie { configure, cache?, champs, confiances, hash }. */
async function extraireActe(buffer, contentType, referentielBanques, cacheGet, cacheSet) {
  ...
  if (cacheGet) { try { var hit = await cacheGet(cle); if (hit && hit.resultat) return { configure: true, cache: true, hash: hash, champs: hit.resultat.champs, confiances: hit.resultat.confiances }; } catch (e) {} }
  ...
  return { configure: true, cache: false, hash: hash, champs: res.champs, confiances: res.confiances };
}
